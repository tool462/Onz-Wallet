...
export class VotingPage {
  ...
  loadVoting() {
  	this.votedDelegates = [''];
  	this.accountData.getVotes(this.accountID).then((votes) => {
	  	if (votes['delegates']) {
	  	...
	  	this.accountData.getActiveDelegates(101).then((delegates) => {
  ...
  votePOOL() {
  	...
  	for (let i=0;i < this.poolMembers.length; i++) {
  		if (this.votedDelegates.indexOf(this.poolMembers[i]) == -1) {
  			this.updateVotesArray.push(`+${this.poolMembers[i]}`);
  			this.updateVotesNamesArray.push(`+${this.poolMemberNames[i]}`);
  			// let name = this.shownDelegates.filter(x => x['publicKey'] == this.poolMembers[i]); 
  			// if (name.length > 0) {
  			// 	this.updateVotesNamesArray.push(`+${name[0]['username']}`);
  			// } else {
  			// 	this.updateVotesNamesArray.push(`+${this.poolMembers[i]}`);
  			// }
  			
  		}
  	}
  	let remaining = 101-(this.votedDelegates.length-1);
  	if (remaining < this.updateVotesArray.length-1) {
  		this.showToast(`Not enought free slots to vote all pool members. Have ${remaining} and need ${this.updateVotesArray.length-1}.`);
  		this.updateVotesArray = [''];
  		this.updateVotesNamesArray = [''];
  	} else {
