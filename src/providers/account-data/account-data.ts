...
export class AccountDataProvider {
  ...
  init(): Promise<void> {
    this.secureStorage.create('onz_tool_pin')
    ...
    return this.secureStorage.create('onz_tool_accounts')
      .then((storageONZ: SecureStorageObject) => { 
      	this.LOGIN_STORAGE = storageONZ;
        return storageONZ.keys()
          .then(
            data => { 
              let ssData = data;
              var promises = [];
              for (let i=0;i<ssData.length; i++) {
                  promises.push(storageONZ.get(ssData[i]));
              }
  ...
  login(password: string, loginType: string): void {
    if (loginType == "Account") {
	    ...
	    this.KEY_PAIR = onz.crypto.getKeys(password);
	    this.PUBLIC_KEY = this.KEY_PAIR['publicKey'];

	    const accountID = onz.crypto.getAddress(this.PUBLIC_KEY);
  ...
  getAccount(accountID: string = this.ACCOUNT_ID): Promise<object[]> {
      ...
      onz.api(this.OPTIONS).getAccount(accountID, resolve);
    });
  };

  getAccountTransactions(address: string, limit: number, offset: number): Promise<object[]> {
	  return new Promise(resolve => {
	    onz.api(this.OPTIONS).listTransactions(address, limit, offset, resolve);
	  });
  };

  getVotes(address: string) {
	return new Promise(resolve => {
      onz.api(this.OPTIONS).listVotes(address, resolve);
  	});
  };

  getActiveDelegates(amount: number) {
	return new Promise(resolve => {
      onz.api(this.OPTIONS).listActiveDelegates(amount, resolve);
  	});
  };

  getStandbyDelegates(amount: number) {
	return new Promise(resolve => {
      onz.api(this.OPTIONS).listStandbyDelegates(amount, resolve);
  	});
  };

  sendRequest(request: string, parameters: object) {
	return new Promise(resolve => {
      onz.api(this.OPTIONS).sendRequest(request, parameters, resolve);
  	});
  };

  searchDelegates(username: string) {
	return new Promise(resolve => {
      onz.api(this.OPTIONS).searchDelegateByUsername(username, resolve);
  	});
  };

  getTransaction(tx: string) {
  	return new Promise(resolve => { console.log(tx);
  		onz.api(this.OPTIONS).getTransaction(tx, resolve);
  	});
  }
  ...
  setNode(node: string): Promise<void> {
    if (node == 'mainnet/') {
    	...
    	this.OPTIONS = { ssl: true, node: 'tnode11.onzcoin.com', port: '443', testnet: true, randomPeer: false };
    } else {
    	let url;
    	let port;
    	if (node.indexOf(':') > 0) {
  ...
  convertPasswordToAccount(password): string {
    const pKey = onz.crypto.getKeys(password)['publicKey'];

    const accountID = onz.crypto.getAddress(pKey);
    return accountID;
  }
  ...
  voteDelegates(delegates: string[], secondPass: string = null, password: string = this.PASSWORD): Promise<any> {
  	...
  	if (this.HAS_SECOND_PASSWORD) {
  		transaction = onz.vote.createVote(password, delegates, secondPass, 20);
  	} else {
  		transaction = onz.vote.createVote(password, delegates, null, 20);	
  	}
  	console.log(transaction);
  	return this.broadcastTX(transaction);
  }

  sendOnz(to: string, amount: string, secondPass: string = null, password: string = this.PASSWORD): Promise<any> {
  	let transaction;
  	...
  	if (this.HAS_SECOND_PASSWORD) {
  		transaction = onz.transaction.createTransaction(to, amountNum, password, secondPass, 20);
  	} else {
  		transaction = onz.transaction.createTransaction(to, amountNum, password, null, 20);
  	}
  	console.log(transaction);
  	return this.broadcastTX(transaction);
  }
  ...
  broadcastTX(tx: object, setBroadcastNode: boolean = false, broadcastNode: string = ''): Promise<any>{
    ...
    let request = {
        requestMethod: 'POST',
        requestUrl: onz.api(this.OPTIONS).getFullUrl() + '/peer/' + 'transactions',
        nethash: onz.api(this.OPTIONS).nethash,
        requestParams: { transaction: tx }
    };

    return onz.api(this.OPTIONS).doPopsicleRequest(request).then(function (result) {
        return result['body'];
    });
  ...
  getDelegate(pKey: string = this.PUBLIC_KEY): Promise<any>{
  	let request = {
        requestMethod: 'GET',
        requestUrl: onz.api(this.OPTIONS).getFullUrl() + '/api/' + `delegates/get?publicKey=${pKey}`,
    };
  ...
  signTransaction(unsignedTransactionBytes: string): string  {
    return onz.crypto.sign(unsignedTransactionBytes, this.KEY_PAIR);
  }
}